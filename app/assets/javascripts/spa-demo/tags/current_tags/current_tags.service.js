(function() {
  ...
  angular
    ...
    .service("spa-demo.tags.currentTags", CurrentTags);

  CurrentTags.$inject = ["$rootScope","$q",
                             "$resource",
                             "spa-demo.tags.thingType",
                             "spa-demo.config.APP_CONFIG"];

  function CurrentTags($rootScope, $q, $resource, thingType, APP_CONFIG) {
    var tagsResource = $resource(APP_CONFIG.server_url + "/api/tags",{},{
    ...
    service.hasTag = hasTag;
    service.nextThing = nextThing;
    service.previousThing = previousThing;

    //refresh();
    $rootScope.$watch(function(){ return thingType.getVersion(); }, refresh);
    return;
    ////////////////
    function refresh() {      
      var params={};
      ...
      if (thingType.getTag()) {
        params["search_tag"]=thingType.getTag();
      }
    ...
    function refreshImages(params) {
      var result=tagsResource.query(params);
      result.$promise.then(
        function(images){
    ...
    function hasTag() {
      console.log('Tag:', thingType.getTag())
      thingType.getTag() ? true : false;
    }
  ...
  CurrentTags.prototype.getVersion = function() {
    return this.version;
  }
  CurrentTags.prototype.getImages = function() {
    return this.images;
  }
  CurrentTags.prototype.getThings = function() {
    return this.things;
  }
  CurrentTags.prototype.getCurrentImageIndex = function() {
     return this.imageIdx;
  }
  CurrentTags.prototype.getCurrentImage = function() {
    return this.images.length > 0 ? this.images[this.imageIdx] : null;
  }
  CurrentTags.prototype.getCurrentThing = function() {
    return this.things.length > 0 ? this.things[this.thingIdx] : null;
  }
  CurrentTags.prototype.getCurrentImageId = function() {
    var currentImage = this.getCurrentImage();
    return currentImage ? currentImage.image_id : null;
  }
  CurrentTags.prototype.getCurrentThingId = function() {
    var currentThing = this.getCurrentThing();
    return currentThing ? currentThing.thing_id : null;
  }


  CurrentTags.prototype.setCurrentImage = function(index, skipThing) {
  ...
  CurrentTags.prototype.setCurrentThing = function(index, skipImage) {
  ...
  CurrentTags.prototype.setCurrentThingForCurrentImage = function() {
  ...
  CurrentTags.prototype.setCurrentImageForCurrentThing = function() {
    var image=this.getCurrentImage();
    var thing=this.getCurrentThing();
    if (!thing) {
      this.imageIdx=null;
  ...
  CurrentTags.prototype.setCurrentImageId = function(image_id, skipThing) {
  ...
  CurrentTags.prototype.setCurrentThingId = function(thing_id, skipImage) {
  ...
  CurrentTags.prototype.setCurrentTagId = function(thing_id, image_id) {
    console.log("setCurrentTag", thing_id, image_id);
