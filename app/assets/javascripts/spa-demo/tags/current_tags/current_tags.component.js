(function() {
  ...
  angular
    ...
    .component("sdCurrentTagsMap", {
      template: "<div id='map'></div>",
      controller: CurrentTagsMapController,
      bindings: {
        zoom: "@"
      }
    });

  CurrentTagsMapController.$inject = ["$scope", "$q", "$element",
                                          "spa-demo.geoloc.currentOrigin",
                                          "spa-demo.geoloc.myLocation",
                                          "spa-demo.geoloc.Map",
                                          "spa-demo.tags.currentTags",
                                          "spa-demo.config.APP_CONFIG"];
  function CurrentTagsMapController($scope, $q, $element, 
                                        currentOrigin, myLocation, Map, currentTags, 
                                        APP_CONFIG) {
    var vm=this;

    vm.$onInit = function() {
      console.log("CurrentTagsMapController",$scope);
    }
    vm.$postLink = function() {
      ...
      $scope.$watch(
        function(){ return currentTags.getCurrentImage(); }, 
        function(link) { 
          if (link) { 
      ...
      $scope.$watch(
        ...
        function(marker) { 
          if (marker) {
            ...
            currentTags.setCurrentTagId(marker.thing_id, marker.image_id);
          }
        }); 
      $scope.$watch(
    ...
    function displayTag(ti) {
      ...
      if (ti.thing_id && ti.priority===0) {
        ...
        markerOptions.icon = APP_CONFIG.thing_type_marker;
  ...
  CurrentTagsMapController.prototype.updateOrigin = function() {
  ...
  CurrentTagsMapController.prototype.setActiveMarker = function(thing_id, image_id) {
    if (!this.map) { 
      return; 
  ...
  CurrentTagsMapController.prototype.originInfoWindow = function(location) {
  ...
  CurrentTagsMapController.prototype.thingInfoWindow = function(ti) {
  ...
  CurrentTagsMapController.prototype.imageInfoWindow = function(ti) {
